...
const UserAddModal: FC<UserAddModalProps> = ({ open, onClose, onUserAdded }) => {
  ...
  const [authMethod, setAuthMethod] = useState<'email' | 'direct'>('email')
  ...
  const validate = () => {
    ...
    if (authMethod === 'direct' && (!form.password || form.password.length < 8)) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await AdminUserFetch.addUser({
        ...
        password: authMethod === 'direct' ? trimmedForm.password : null,
        role: trimmedForm.role,
        requireEmailVerification: authMethod === 'email'
      })

      if (result.success && result.data) {
        if (authMethod === 'email') {
          setSuccess('ユーザを追加し、認証メールを送信しました')
        } else {
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <fieldset disabled={completed || isSubmitting}>
              <div className="grid grid-cols-4 gap-4">
                ...
                {/* 認証方式選択 */}
                <div className="col-span-4">
                  <label className="mb-2 block font-medium text-gray-900">認証方式</label>
                  <div className="flex gap-4">
                    <label className="flex cursor-pointer items-center">
                      <input
                        ...
                        onChange={(e) => setAuthMethod(e.target.value as 'email' | 'direct')}
                        className="mr-2"
                      />
                      <span className="text-gray-900">メール認証</span>
                    </label>
                    <label className="flex cursor-pointer items-center">
                      <input
                        type="radio"
                        ...
                        className="mr-2"
                      />
                      <span className="text-gray-900">直接パスワード設定</span>
                    </label>
                  </div>
                  <p className="mt-1 text-sm text-gray-600">
                    {authMethod === 'email'
                      ? 'ユーザがメールからパスワードを設定します'
                      : '管理者がパスワードを設定します'}
                  </p>
                </div>
                  ...
                  <div className="col-span-4">
                    ...
                    <div className="relative">
                      <input
                        ...
                        value={form.password}
                        onChange={handleChange}
                        className="focus:border-primary-500 focus:ring-primary-500 block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 pr-10 text-gray-900"
                        required={authMethod === 'direct'}
                        minLength={8}
                      />
                      <button
                        type="button"
